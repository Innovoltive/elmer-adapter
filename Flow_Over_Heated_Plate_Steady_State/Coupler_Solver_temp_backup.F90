MODULE HelperMethods
    ...
    SUBROUTINE Print(dataName,mesh,BoundaryPerm,CoordVals)
        ...
        DO i = 1, mesh % NumberOfNodes
            ...
            IF(j == 0) CYCLE
            IF (meshDim == 2) THEN
                write(infoMessage,'(A,I5,A,I5,A,F10.4,A,F10.6,A,F10.6)') 'Node: ',i,' Index: ',j,' Value: ' &
                            ,dataVariable % Values(dataVariable % Perm(i)),&
                            ' X= ', CoordVals(meshDim * j-1), ' Y= ', CoordVals(meshDim * j) 
            ELSE IF (meshDim == 3) THEN
                write(infoMessage,'(A,I5,A,I5,A,F20.4,A,F10.6,A,F10.6,A,F10.6)') 'Node: ',i,' Index: ',j,' Value: ' &
                            ,dataVariable % Values(dataVariable % Perm(i)),&
    ...
    SUBROUTINE CopyWriteData(dataName,mesh,BoundaryPerm,copyData)
        ...
        DO i = 1, mesh % NumberOfNodes
            ...
            ! For Debugging purpose uncomment the below line
            ! WRITE(*,*) 'Heat Flux:',copyData(j)
    ...
    case(1)
        ...
        DO i=1,Mesh % NumberOfNodes
            ...
            ELSE IF (meshDim == 3) THEN
                ...
                ! PRINT *, 'X:',CoordVals(meshDim *j-2),' Y:',CoordVals(meshDim *j-1),' Z:',CoordVals(meshDim *j)              
            END IF            
        END DO
